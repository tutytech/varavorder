...
import 'package:orderapp/editgroup.dart';
...
class Grouplist extends StatefulWidget {
  final String? rights;
  Grouplist({Key? key, this.rights}) : super(key: key);

  @override
  _BranchListPageState createState() => _BranchListPageState();
}

class _BranchListPageState extends State<Grouplist> {
  ...
  Future<void> deleteGroup(BuildContext context, String branchId) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final String userId = prefs.getString('userId') ?? "";

    const String apiUrl = 'https://varav.tutytech.in/group.php';
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.isNotEmpty && responseData[0]['status'] == 'success') {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                responseData[0]['message'] ?? 'Group deletion failed.',
              ),
            ),
          );
        }
                                  ...
                                  _filteredBranches.map((branch) {
                                    return DataRow(
                                      cells: [
                                        ...
                                        DataCell(
                                          Text(branch['groupname'] ?? '0'),
                                        ),
                                        DataCell(
                                          Text(branch['groupcode'] ?? 'N/A'),
                                        ),
                                        DataCell(
                                          Text(branch['description'] ?? 'N/A'),
                                        ),
